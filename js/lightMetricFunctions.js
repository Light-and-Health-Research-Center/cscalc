function rdivide(a, b) {
  if (a.length != b.length) {
    console.log(
      a + " and " + b + " are not the same length and can not be rdivided."
    );
    return;
  }
  var ret = [];
...
function elementWiseDivision(a, b) {
  if (b.length != 1) {
    console.log(
      b +
        "'s length is not equal to 1 and element-wise division cannot be performed."
    );
  }
  var ret = [];
  for (var i = 0; i < a.length; i++) {
...
function elementWiseMultiplication(a, b) {
  if (b.isNaN) {
    console.log(
      b +
        "'s length is not equal to 1 and element-wise multiplication cannot be performed."
    );
  }
  var ret = [];
  for (var i = 0; i < a.length; i++) {
...
function CLAcalc(spd) {
  ...
  var CIE_S_cone_opic_irr = sumproduct(
    values,
    arrayMul(deltaWavelength, efs.CIE_S_cone)
  );
  var CIE_M_cone_opic_irr = sumproduct(
    values,
    arrayMul(deltaWavelength, efs.CIE_M_cone)
  );
  var CIE_L_cone_opic_irr = sumproduct(
    values,
    arrayMul(deltaWavelength, efs.CIE_L_cone)
  );
  var CIE_rhodopic_irr = sumproduct(
    values,
    arrayMul(deltaWavelength, efs.CIE_Rhodopic)
  );
  var CIE_melanopic_irr = sumproduct(
    values,
